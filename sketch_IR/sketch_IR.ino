//define pins. I used pins 4 and 5
#define irLedPin 14          // IR Led on this pin
#define irSensorPin 15       // IR sensor on this pin
int value[10];
int irOutPin = 2;
int irInPin = 4;
unsigned long throwTime, bounceTime;

int readIR(int times);
int irRead(int readPin, int triggerPin); //function prototype

void setup()
{
  pinMode(irSensorPin, INPUT);
  pinMode(irLedPin, OUTPUT);
  
  pinMode(irOutPin, OUTPUT);
  pinMode(irInPin, INPUT);  
  
  digitalWrite(irOutPin, LOW);
  digitalWrite(irInPin, LOW);
  
  Serial.begin(9600); 
  // prints title with ending line break 
  Serial.println("Program Starting"); 
  // wait for the long string to be sent 
  delay(100); 
}

void loop()
{ 
  //Serial.println(irRead(irSensorPin, irLedPin)); //display the results
  //delay(10);
  //Serial.println(readIR(5));
  //delay(10); //wait for the string to be sent
  Serial.println(irBounce());
  delay(50);
}


int irBounce()
{
  int distance = 0;
  throwTime = micros();
  digitalWrite(irOutPin, HIGH);
  if(digitalRead(irInPin)==1)
  {
    bounceTime = micros();
    distance = bounceTime - throwTime;
  }
  //digitalWrite(irOutPin, LOW);
  return distance;  
}
  

int readIR(int times){
  int ambientIR;                // variable to store the IR coming from the ambient
  int obstacleIR;               // variable to store the IR coming from the object
  int distance;                 // variable that will tell if there is an obstacle or not
  
  for(int x=0;x<times;x++){     
    digitalWrite(irLedPin,LOW);           // turning the IR LEDs off to read the IR coming from the ambient
    //delay(1);                                             // minimum delay necessary to read values
    ambientIR = analogRead(irSensorPin);  // storing IR coming from the ambient
    delay(30);
    digitalWrite(irLedPin,HIGH);          // turning the IR LEDs on to read the IR coming from the obstacle
    //delay(1);                                             // minimum delay necessary to read values
    obstacleIR = analogRead(irSensorPin);  // storing IR coming from the obstacle
    value[x] = ambientIR-obstacleIR;   // calculating changes in IR values and storing it for future average
  }
 
  for(int x=0;x<times;x++){        // calculating the average based on the "accuracy"
    distance+=value[x];
  }
  return(distance);            // return the final value
}

/******************************************************************************
 * This function can be used with a panasonic pna4602m ir sensor
 * it returns a zero if something is detected by the sensor, and a 1 otherwise
 * The function bit bangs a 38.5khZ waveform to an IR led connected to the
 * triggerPin for 1 millisecond, and then reads the IR sensor pin to see if
 * the reflected IR has been detected
 ******************************************************************************/
int irRead(int readPin, int triggerPin)
{
  int halfPeriod = 13; //one period at 38.5khZ is aproximately 26 microseconds
  int cycles = 38; //26 microseconds * 38 is more or less 1 millisecond
  int i;
  unsigned long timethrow, timebounce=0;
  
  timethrow = micros();
  for (i=0; i <=cycles; i++)
  {
    digitalWrite(triggerPin, HIGH); 
    delayMicroseconds(halfPeriod);
    digitalWrite(triggerPin, LOW); 
    delayMicroseconds(halfPeriod - 1);     // - 1 to make up for digitaWrite overhead    
  }
  
  if(1==digitalRead(readPin))
    timebounce = micros() - timethrow;
    
    return timebounce;    
}
